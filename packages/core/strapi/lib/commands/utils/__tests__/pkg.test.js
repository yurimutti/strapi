...
describe('pkg', () => {
  ...
  describe('validatePkg', () => {
    it("should return the validated package.json if it's valid", async () => {
      ...
      const validatedPkg = await validatePkg({ pkg, logger: loggerMock });
    ...
    it('should fail if a required field is missing and call the error logger with the correct message', async () => {
      ...
      expect(pkg1).toBeNull();

      expect(loggerMock.error.mock.calls[0]).toMatchInlineSnapshot(`
        [
          "'name' in 'package.json' is required as type '[35mstring[39m'",
        ]
      `);

      const pkg2 = await validatePkg({
        pkg: {
          name: 'testing',
        },
        logger: loggerMock,
      ...
      expect(loggerMock.error.mock.calls[1]).toMatchInlineSnapshot(`
        [
  ...
  describe('parseExportsOrdering', () => {
    it('should throw if there are no exports at all and log that error', async () => {
      ...
      await expect(
        ...
        `"'package.json' must contain either a 'main', 'module' or 'exports' property"`
      );
    });
